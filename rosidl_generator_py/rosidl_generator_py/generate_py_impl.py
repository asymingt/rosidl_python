    ...
    for subfolder in modules.keys():
        ...
        for module_name, idl_stem in sorted((value, key) for (key, value) in module_names.items()):
            ...
            type_support_impl_by_filename = {
                '%s_s.ep.{0}.c'.format(impl): impl for impl in typesupport_impls
            }
            ...
            for template_file, generated_filenames in mapping_msg_pkg_extension.items():
                for generated_filename in generated_filenames:
                    data = {
                        'package_name': f"{package_name}__{subfolder}_{module_name}",
                        'content': idl_content,
                        'typesupport_impl': type_support_impl_by_filename.get(generated_filename, ''),
                    }
                    generated_file = os.path.join(
                        args['output_dir'], generated_filename % f"{subfolder}/{module_name}"
                    )
                    expand_template(
